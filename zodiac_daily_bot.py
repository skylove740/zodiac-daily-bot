...
def get_daily_star_fortunes():
    ...
    prompt = (
        "오늘 날짜의 12 별자리 운세를 각각 한 문단으로 써줘. 별자리 순서는 다음과 같아:\n" + ", ".join(ZODIACS_star) +
        "\n12개 별자리를 하나도 빼놓지 말고 한 문단씩 써줘야해.\n말투는 유튜브 쇼츠에 잘 어울리는 스타일이면 좋아.\n각 별자리별 운세를 줄바꿈으로 문단을 확실히 나눠줘. 예시는 다음과 같아.\n"
        + """
물병자리 
작은 선택이 큰 변화를 가져올 수 있는 하루예요. 망설이지 말고 마음 가는 길을 따라가 보세요. 오늘의 당신은 충분히 멋져요. 
...
이 형식으로 별자리별로 한 문단씩 12개 별자리 운세를 모두 줘.
""")
    res = client.chat.completions.create(model="gpt-3.5-turbo",
                                         messages=[{
                                             "role": "user",
